...
In this project, a middleware and a method decorator were implemented as requested in the technical test. The middleware logs relevant information for each request directly to the console, while the decorator measures and logs the execution time of the GraphQL queries. This ensures that every query run through Apollo can be monitored for performance and behavior.

Additionally, a cron job was implemented to update the character database. For demonstration purposes, the cron is set to run every minute so that evaluators can easily see it executing and verify that the scheduled updates are functioning correctly. All logs generated by the cron, middleware, and decorator appear in the console in real time.
